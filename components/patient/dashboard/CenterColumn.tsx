...
export default function CenterColumn({ displayName, upcomingAppointments }: { displayName: string, upcomingAppointments: any[] }) {
    ...
    return (
        <div className="flex-1 h-full overflow-y-auto w-full px-10 py-10 bg-[#fafdfc]">
                        ...
                        <div className="flex-1 w-full">
                            <div className="flex justify-between items-start w-full">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden ring-2 ring-white shadow-sm flex items-center justify-center">
                                        <span className="text-lg">👩🏻‍⚕️</span>
                                    </div>
                                    <div>
                                        <h5 className="font-bold text-sm text-gray-900">{nextApt.doctor.user.name}</h5>
                                        <p className="text-xs text-gray-500 font-medium">{nextApt.doctor.specialty || "Therapy"}</p>
                                    </div>
                                    {nextApt.meetLink ? (
                                        <a href={nextApt.meetLink} target="_blank" rel="noreferrer" className="ml-4 bg-[#98c99f] text-white px-5 py-2.5 rounded-2xl text-xs font-bold shadow-sm hover:opacity-90">
                                            Video call
                                        </a>
                                    ) : null}
                                </div>
            ...
            {/* Patient Activities & Progress Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6">
                <div>
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-gray-900">Patient activities</h3>
                        ...
                        <div className="h-[180px] w-full flex items-end justify-between px-4 pb-2">
                            {chartData.map((d, i) => (
                                <div key={i} className="flex flex-col items-center gap-3 w-10">
                                    <div
                                        className={`w-full rounded-t-full transition-all duration-500 ${d.active ? 'bg-[#98c99f]' : 'bg-[#ddebe0]'}`}
                    ...
                    <Link href="/patient/mood" className="mt-6 bg-white p-5 rounded-[24px] shadow-[0_2px_20px_-5px_rgba(0,0,0,0.05)] border border-gray-50/50 flex items-center justify-between group cursor-pointer hover:border-[#98c99f] transition-colors">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-blue-50 text-blue-500 shrink-0 rounded-2xl flex items-center justify-center text-xl shadow-sm border border-blue-100">
                                💙
                            </div>
                            <div>
                                <h4 className="font-bold text-gray-900">Good conditions</h4>
                                <p className="text-xs text-gray-500 font-medium">Anxiety & wellness</p>
                            </div>
                        </div>
                ...
                <div>
                    {/* Daily Progress */}
                    <div className="h-full bg-gradient-to-br from-[#eff8f1] to-[#e1f0e6] p-8 rounded-[30px] border border-[#d2efe0] flex flex-col items-center text-center">
                        <h3 className="text-lg font-bold text-gray-900">Daily progress</h3>
                        <p className="text-xs text-green-800/70 font-medium mt-2 max-w-[120px]">Keep improving the quality of your health</p>
                        ...
                        <div className="mt-8 relative w-36 h-36 flex items-center justify-center">
                            ...
                            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                                ...
                                <circle cx="50" cy="50" r="40" className="stroke-[#98c99f] fill-none drop-shadow-md" strokeWidth="12" strokeDasharray="251.2" strokeDashoffset="50.24" strokeLinecap="round" />
                            </svg>
                            <div className="absolute inset-0 flex items-center justify-center flex-col">
                                <span className="text-3xl font-extrabold text-[#2d5945]">80%</span>
                            </div>
                        </div>
                    </div>
                </div>
