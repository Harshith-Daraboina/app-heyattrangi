...
The patient **AI Bot** page calls `POST /api/pragya/chat`, which forwards to `{BOT_API_URL}/chat` with body `{ session_id, message }`. **End & Summarize** uses `POST /api/pragya/summary` → `{BOT_API_URL}/summary` with `{ session_id }` (same base URL). If `BOT_API_URL` is unset, the app defaults to **Bot-Heyattrangi-V4** on Hugging Face (`https://heyattrangi-spaces-bot-heyattrangi-v4.hf.space`).
